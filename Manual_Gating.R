...
x <- subset(opp, pop==0)
plot(x[,para], pch=16, cex=0.4, col = densCols(x[,para], colramp = .rainbow.cols), main="Gating BEADS")
...
### SYN
para <- c("fsc_small","pe")
plot(x[,para], pch=16, cex=0.4, col = densCols(x[,para], colramp = .rainbow.cols), main="Gating Synecho")
