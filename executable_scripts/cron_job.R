...
for(i in phyto)	try(plot.map (stat, popname=i, param='abundance'),silent=TRUE)
dev.off()


png("cell_conc_time.png",width=15, height=9, unit='in', res=300)
par(mfrow=c(ceiling(length(phyto)/2),2), cex=cex, mar=c(2,2,2,3), oma=c(1,1,3,0))
for(i in phyto)	try(plot.time(stat, popname=i, param='abundance'),silent=TRUE)
dev.off()


png("~/light_scatter.png",width=15,height=9,unit='in',res=300)
par(mfrow=c(ceiling(length(phyto)/2),2), cex=cex, mar=c(2,2,2,3), oma=c(1,1,3,0))
for(i in phyto)	try(plot.time(stat, popname=i, param='fsc_small'),silent=TRUE)
dev.off()


png("~/chl_small.png",width=15,height=9,unit='in',res=300)
par(mfrow=c(ceiling(length(phyto)/2),2), cex=cex, mar=c(2,2,2,3), oma=c(1,1,3,0))
for(i in phyto)	try(plot.time(stat, popname=i, param='chl_small'),silent=TRUE)
dev.off()
