...
write.csv(poly, paste(gating.param.location_archived, "/",popname,".",time,".csv",sep=""), quote=FALSE, row.names=FALSE) # TODO(hyrkas): add archive_folder for Params in Globals
...
list.params <- list.files(path=gating.param.location, pattern= ".csv", full.names=TRUE)
