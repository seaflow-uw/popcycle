...
#' @param plot.beads Do you want to plot beads signal over time for fsc_small, chl_small and pe? (default is TRUE)
#' @return A dataframe with normalized fsc_small, chl_small and pe values
#' @export
normalization <- function(stat, spar=0.5, plot.beads=T){
  ...
  beads <- subset(stat, pop =='beads')[,c("time", "fsc_small","chl_small","pe")]
  phyto <- subset(stat, pop !='beads' & pop !='unknown')

  # Smooth the beads data using the spar value
    #fsc
  smooth <- smooth.spline(beads$time, beads$fsc_small,spar=spar)
  smooth.beads.fsc <- spline(smooth$x, smooth$y, xout=unique(phyto$time))
    #chl
  smooth <- smooth.spline(beads$time, beads$chl_small,spar=spar)
  smooth.beads.chl <- spline(smooth$x, smooth$y, xout=unique(phyto$time))
    #pe
  smooth <- smooth.spline(beads$time, beads$pe,spar=spar)
  smooth.beads.pe <- spline(smooth$x, smooth$y, xout=unique(phyto$time))

  clean.beads <- data.frame(cbind(time=as.POSIXct(smooth.beads.fsc$x, origin="1970-01-01", tz="GMT"), fsc.b=smooth.beads.chl$y, chl.b=smooth.beads.fsc$y, pe.b=smooth.beads.pe$y))

  #  Plot the light scattering of beads over time
  if(plot.beads == TRUE){
    par(mfrow=c(3,1),cex=1.2, mar=c(2,5,1,1), oma=c(1,1,1,1))
    plot(beads$time, beads$fsc_small, ylim=c(1, 10^3.5), log='y', xlab=NA, ylab="fsc_small", main='beads')
    lines(smooth.beads.fsc, col=2, lwd=3)
    plot(beads$time, beads$chl_small, ylim=c(1, 10^3.5), log='y', xlab=NA, ylab="chl_small")
    lines(smooth.beads.chl, col=2, lwd=3)
    plot(beads$time, beads$pe, ylim=c(1, 10^3.5), log='y', xlab=NA, ylab="pe")
    lines(smooth.beads.pe, col=2, lwd=3)
  }

  t <- merge(clean.beads, phyto, by="time", all.y=T)
  phyto$norm.fsc <- t$fsc_small/t$fsc.b
  phyto$norm.chl <- t$chl_small/t$chl.b
  phyto$norm.pe <- t$pe/t$pe.b
...
#'
#' @param phyto Table generated by the function normalization().
#' @param inst Serial number of the instrument, either 740 or 751
#' @return A dataframe with carbon cell quotas and biomass
#' @export
carbon_conversion <- function(phyto, inst){
