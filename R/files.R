...
flowrate <- function(stat, inst=NULL){
  ...
  load(system.file("flowrate", paste0("lm_",inst),package='popcycle'))
    fr <- predict(reg, newdata=data.frame(measured.pressure=log10(stat$stream_pressure)),interval='predict', se.fit=T)

    stat$flow_rate_mL.min <- 10^fr$fit[,1]
    stat$flow_rate_se_mL.min <- 10^fr$se.fit
...
#' @examples
#' \dontrun{
#' stats <- get.stat.table(db, inst=NULL, flag=F)
#' }
#' @export
get.stat.table <- function(db, inst=NULL, flag=FALSE) {
  ...
  DF <- get.raw.stat.table(db, flag=flag)
  stat <- flowrate(DF, inst=inst)
  stat[,c("abundance_cells.mL")]  <- stat[,"n_count"] / (VC * stat[,"opp_evt_ratio"] * stat[,c("flow_rate_mL.min")] * stat[,"file_duration"]/60)
  stat[,c("abundance_se_cells.mL")]  <- stat[,c("abundance_cells.mL")] * stat[,c("flow_rate_se_mL.min")] / stat[,c("flow_rate_mL.min")]
...
#' @export
normalization <- function(stat, spar=0.75){
...
carbon_conversion <- function(stat, inst=NULL){
  ...
  qc <- predict(reg, newdata=data.frame(norm.fsc=log10(stat$fsc_small)),interval='predict')#, se.fit=T)

  stat[,"Qc_pgC.cell"] <- 10^qc$fit[,1]
  stat[,"Qc_se_pgC.cell"] <- 10^qc$se.fit
  stat[,"Cbiomass_ngC.L"] <- stat[,"Qc_pgC.cell"] * stat[,"abundance_cells.mL"]
  stat[,"Cbiomass_se_ngC.L"] <- stat[,"Cbiomass_ngC.L"] * (stat[,"abundance_se_cells.mL"] / stat[,"abundance_cells.mL"] + stat[,"Qc_se_pgC.cell"] / stat[,"Qc_pgC.cell"])
