...
#' @param norm Median and SD value of beads signal used for the normalization. If NULL (default), median and sd value of beads of the lower quantile from the Stat table will be used as input.
...
normalization <- function(stat, channel="fsc_small", norm=NULL){
  ...
  if(is.null(norm)){
        ...
        b.channel <- median(beads[,channel])
        b.channel.sd <- sd(beads[,channel])

      }
    }else{
      norm <- as.numeric(norm)
      b.channel <- as.vector(norm[1])
      b.channel.sd <- as.vector(norm[2])
    }

  stat[,paste0("norm.",channel)] <- stat[,channel] / b.channel
  stat[,paste0("norm.",channel,".sd")] <- stat[,paste0("norm.",channel)] * b.channel.sd / b.channel
...
#' @export
size.carbon.conversion <- function(stat, inst=NULL){
  ...
  u <- approx(x=log10(mie[,paste0("scatter",inst,quantile,sep="_")]), y=log10(mie$Qc), n=10^3.5)
  v <- approx(x=log10(mie[,paste0("scatter_",inst,quantile,sep="_")]), y=log10(mie$diameter), n=10^3.5)
...
#' Calculate mean and sd from the different quantiles.
#'
#' @param stat Table generated by the function size.carbon.conversion.
#' @return Data frame of aggregate statistics.
#' @export
...
get.clean.stat.table <- function(db, inst=NULL){
  ...
  print("3. Converting normalized light scattering to cell diameter & carbon quotas")
      stat <- size.carbon.conversion(stat, inst=inst)
  print("4. Merging quantiles")
      stat <- combine.quantile(stat)
