...
size_carbon_conversion <- function(stat, inst=NULL){
  ...
  id.u <- findInterval(log10(stat[,"fsc_small"]), u$x)
  id.v <- findInterval(log10(stat[,"fsc_small"]), v$x)
...
merge.stat <- function(stat){
    ...
    para <- c("lat","lon","temp","salinity","conductivity","par","stream_pressure","file_duration","event_rate","opp_evt_ratio","n_count","D1","D2","fsc_small","chl_small","pe","fsc_perp","flow_rate","abundance", "diameter","Qc","Cbiomass")
      df.mean <- aggregate(df[,para], by=list(time=df$time, pop=df$pop), FUN=mean)
      df.mean$file <- stat[match(df.mean$time, stat$time),"file"]

    para <- c("D1","D2","fsc_small","chl_small","pe","fsc_perp","abundance","diameter", "Qc","Cbiomass")
...
get.clean.stat.table <- function(db, inst=NULL, spar=0.1){
  ...
  print("3. Converting normalized light scattering to carbon"); stat <- size_carbon_conversion(stat, inst=inst);
  print("4. Merging quantiles"); stat <- merge.stat(stat)
